...
class MockPaymentGatewayDialog extends StatefulWidget {
  final double amount;
  final int
  paymentMethodIndex; // 0: Card, 1: UPI, 2: Scan, 3: Netbanking, 4: Wallets
  ...
  @override
  State<MockPaymentGatewayDialog> createState() =>
      _MockPaymentGatewayDialogState();
}
  ...
  Future<void> _processPayment() async {
    ...
    if (_currentMethod == 0) {
      if (_cardNumberController.text.length < 10) {
        return _snack('Invalid Card Number');
      }
    } else if (_currentMethod == 1) {
        ...
        child: Container(
          ...
          child: Column(
            ...
            children: [
              ...
              if (_state == 2)
                Expanded(
                  child: SingleChildScrollView(child: _buildOtpOrWaitScreen()),
                ),
              if (_state == 3)
                const Expanded(
                  child: Center(child: CircularProgressIndicator()),
                ),
              if (_state == 4) Expanded(child: _buildSuccessScreen()),
          ...
          if (_currentMethod == 0) ...[
            ...
            const Center(
              child: Padding(
                ...
                child: Text('Redirecting to external wallet...'),
              ),
            ),
          ],
  ...
  Widget _buildOtpOrWaitScreen() {
    ...
    return Padding(
      ...
      child: Column(
        children: [
          Icon(
            isUpi ? Icons.mobile_friendly : Icons.lock_clock,
            size: 64,
            color: themeColor,
          ),
          const SizedBox(height: 16),
          ...
          Text(
            isUpi
                ? 'Scan/Approve request on your UPI app.'
    ...
    return Center(
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 8),
          Text(
            'Merchant Reference: #${DateTime.now().millisecondsSinceEpoch.toString().substring(8)}',
            style: const TextStyle(color: Colors.grey),
          ),
        ],
      ),
